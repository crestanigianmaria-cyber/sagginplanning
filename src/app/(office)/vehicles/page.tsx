    ...
    <div className="h-full flex flex-col max-w-[1600px] mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Parco Mezzi</h1>
            <p className="text-gray-500 mt-1">Gestisci la flotta e verifica le disponibilità</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {vehicles.map((v) => (
          <Link key={v.id} href={`/vehicles/${v.id}`} className="group block">
            <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-xl hover:shadow-[0_0_30px_rgba(220,38,38,0.1)] hover:border-gray-300 transition-all relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-6 relative">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#dc2626] transition-colors">{v.name}</h3>
                  <div className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold bg-gray-100 text-gray-700 mt-2 border border-gray-300 shadow-inner uppercase tracking-wider">
                    {v.licensePlate}
                  </div>
                </div>
              ...
              <div className="pt-4 border-t border-gray-200/80 flex items-center justify-between relative">
                {v.hasCrane ? (
        ...
        {vehicles.length === 0 && (
           <div className="col-span-full py-12 text-center bg-white rounded-2xl border border-gray-200 border-dashed">
             ...
             <p className="text-gray-500 font-medium">Nessun mezzo registrato.</p>
           </div>
        )}
      </div>
    </div>
